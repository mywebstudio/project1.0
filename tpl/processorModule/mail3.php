...
if(!Post("name")) {
    $ret["status"] = 'ERROR';
    $ret["msg"] = "Не указано имя";
    echo json_encode($ret);
}

else if(!Post("email")) {
    $ret["status"] = 'ERROR';
    $ret["msg"] = "Не указан обратный адрес";
    echo json_encode($ret);
}

else if(!Post("text")) {
    $ret["status"] = 'ERROR';
    $ret["msg"] = "Не указан текст сообщения";
    echo json_encode($ret);
}

else if(Post("captcha") != '9' ) {
    $ret["status"] = 'ERROR';
    $ret["msg"] = "Не верно указано число на картике";
    echo json_encode($ret);
}
...
//include("class.smtp.php"); // optional, gets called from within class.phpmailer.php if not already loaded
    ...
    $body = 'Поступила заявка с сайта от человека по имени ' . Post("name") . ' - ' . Post("email") . ' <br>Его номер: ' . Post("phone") . ' и адресс ' . Post("adress") .
        ' <br>Его вопрос: ' . Post("note") . ' <br>Касается проекта: ' . Post("project");
