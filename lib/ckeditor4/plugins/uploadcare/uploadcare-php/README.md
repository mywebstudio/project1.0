...
This will return Uploadcare\File instance.
    ...
    $file = $api->uploader->fromUrl('http://www.baysflowers.co.nz/i/image/tangerine-delight.jpg');
    $file->store();
...
By default, Uploader will make 5 checks max with 1 second wait. You can change these params:
    ...
    $file = $api->uploader->fromUrl('http://www.baysflowers.co.nz/i/image/tangerine-delight.jpg', true, $timeout, $max_attempts);

If file is not uploaded an Exception will be thrown.

You can just get token and check status manually later any time:

    $token = $api->uploader->fromUrl('http://www.baysflowers.co.nz/i/image/tangerine-delight.jpg', false);
