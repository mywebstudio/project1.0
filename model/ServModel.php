...
class ServModel extends Model
{
    ...
    {
        global $g_databases;
        parent::__construct($g_databases->db, "serv", "article_id", $id, $onlyShow);
        $this->filter = new ServFilter($g_databases->db, $this->table);
    }
    ...
    {
        return $this->db->selectCell("SELECT (`article_id`) FROM `serv` WHERE `alias` = '$alias'  and `is_removed` = 0");
    }


    public function Getidfromalias($alias1)
    {
        return $this->db->selectCell("SELECT (`article_id`) FROM `serv` WHERE `alias` = '$alias1'");
    }
