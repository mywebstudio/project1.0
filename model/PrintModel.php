...
class PrintModel extends Model
{
            ...
            $this->db->query("CREATE TABLE IF NOT EXISTS ?#
                                 (
                                   `article_id`  INT NOT NULL AUTO_INCREMENT,
                                    ...
                                    `short`  VARCHAR(512) CHARACTER SET utf8 COLLATE utf8_general_ci,
                                    `alias` VARCHAR(512) CHARACTER SET utf8 COLLATE utf8_general_ci,                                   
                                    `full` TEXT CHARACTER SET utf8 COLLATE utf8_general_ci,                                   
                                    `custom` TEXT CHARACTER SET utf8 COLLATE utf8_general_ci,                                   
    ...
    {
        global $g_databases;
        parent::__construct($g_databases->db, "print", "article_id", $id, $onlyShow);
        $this->filter = new PrintFilter($g_databases->db, $this->table);
    }
    ...
    {
        return $this->db->selectCell("SELECT (`article_id`) FROM `print` WHERE `alias` = '$alias'  and `is_removed` = 0");
    }


    public function Getidfromalias($alias1)
    {
        return $this->db->selectCell("SELECT (`article_id`) FROM `print` WHERE `alias` = '$alias1'");
    }
