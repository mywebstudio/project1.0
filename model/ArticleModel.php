...
class ArticleModel extends Model
{
    ...
    {
        global $g_databases;
        parent::__construct($g_databases->db, "article", "article_id", $id, $onlyShow);
        $this->filter = new ArticleFilter($g_databases->db, $this->table);
    }
    ...
    {
        return $this->db->selectCell("SELECT (`article_id`) FROM `article` WHERE `alias` = '$alias'  and `is_removed` = 0");
    }


    public function Getidfromalias($alias1)
    {
        return $this->db->selectCell("SELECT (`article_id`) FROM `article` WHERE `alias` = '$alias1'");
    }
